...
function AccountMergeRequestContent(): React.JSX.Element {
  ...
  const provider = searchParams.get('provider') as OAuthAccountProviderType;
        ...
        >
          <div className="mb-6 text-left">
            <Alert
              ...
              message={
                <>
                  <strong>{email}</strong> 계정 소유자에게 확인 이메일이 발송되었습니다. 소유자가
                  승인하면 계정이 병합됩니다.
                  {mergeMutation.data?.requestId && (
                    <span className="block text-xs text-blue-600 mt-2">
                      요청 ID: {mergeMutation.data.requestId}
                    </span>
                  )}
                </>
              }
